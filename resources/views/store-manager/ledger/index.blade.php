                        ...
                        <form method="GET" action="{{ route('store.ledger.index') }}" class="row g-2">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="invoice_no" value="{{ request('invoice_no') }}"
                                    class="form-control" placeholder="INV-2026...">
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="user_type" class="form-select">
                                    ...
                                    @foreach ($userTypes as $userType)
                                        <option value="{{ $userType }}" @selected(request('user_type') === $userType)>{{ $userType }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <table class="table table-striped table-hover align-middle mb-0">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th>Debit</th>
                                        <th>Close</th>
                                        <th>Credit Employee</th>
                                        <th>Debit Employee</th>
                                        {{-- <th>User Type</th> --}}
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($ledgerEntries as $entry)
                                        <tr>
                                            ...
                                            <td>{{ $entry->emp_user_name ?: ($entry->emp_first_name ?: 'ID: ' . $entry->emp_id) }}
                                            </td>
                                            ...
                                            <td>{{ $entry->credit_user_name ?: ($entry->credit_first_name ?: ($entry->credit_emp_id ? 'ID: ' . $entry->credit_emp_id : '—')) }}
                                            </td>
                                            <td>{{ $entry->debit_user_name ?: ($entry->debit_first_name ?: ($entry->debit_emp_id ? 'ID: ' . $entry->debit_emp_id : '—')) }}
                                            </td>
                                            {{-- <td>{{ $entry->UserType ?: '—' }}</td> --}}
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="11" class="text-center text-muted py-4">No ledger entries found.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
